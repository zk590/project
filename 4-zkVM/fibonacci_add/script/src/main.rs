...
const ELF: &[u8] = include_elf!("fibonacci-add-program");
...
fn main() {
    ...
    // 从文件中反序列化斐波那契结果数据
    let fibonacci_result = read_and_deserialize(FIBONACCI_DATA_FILE).expect("无法从文件中反序列化数据");
    ...
    // Serialize proof, public values and verification key hash to files
    std::fs::write("proof.bin", proof.bytes()).expect("failed to write proof to file");
    std::fs::write("public_values.bin", proof.public_values.to_vec()).expect("failed to write public values to file");
    std::fs::write("vk_hash.bin", vk.bytes32().as_bytes()).expect("failed to write vk hash to file");
    println!("serialized proof, public values and verification key hash to files");
    ...
    println!("successfully generated and verified proof for the fibonacci addition program!")
}