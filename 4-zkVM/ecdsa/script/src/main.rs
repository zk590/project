...
const ELF: &[u8] = include_elf!("ecdsa-program");
...
// 从文件读取并使用rkyv反序列化
fn read_and_deserialize(file_path: &str) -> Result<EcdsaResults, IoError> {
    // 检查文件是否存在
...
fn main() {
    ...
    // 从文件中反序列化批量ECDSA结果数据
    let ecdsa_results = read_and_deserialize(ECDSA_DATA_FILE).expect("无法从文件中反序列化数据");
