...
fn main() {
    ...
    // 从文件中反序列化批量哈希结果数据
    let file_path = args.file_path.as_deref().unwrap_or("/opt/project/1-Sender/sha3/sha3_batch_hash.bin");
    ...
    if args.execute {
        ...
        let (output, report) = client.execute(SHA3_ELF, stdin).run().unwrap();
        ...
        // 写入文件
        let output_file = "sha3_proof_data.bin";
