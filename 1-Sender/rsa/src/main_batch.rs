use clap::Parser;
use rkyv::{Archive, Serialize, Deserialize};
use rsa::{pkcs8::DecodePrivateKey, Pkcs1v15Sign, RsaPrivateKey};
use sha2::{Digest, Sha256};
use hex;
use std::fs::File;
use std::io::{Write, Read, BufRead, BufReader};
use std::path::Path;
use std::time::Instant;

use common::constants::RSA_HASH_BATCH_FILE;

// 定义命令行参数结构
#[derive(Parser, Debug)]
#[command(author, version, about, long_about = None)]
struct Args {
    /// 包含多行消息的输入文件路径
    #[arg(short, long)]
    input_file: String,
}

// 定义单个签名结果数据结构
#[derive(Archive, Serialize, Deserialize, Debug)]
#[archive(check_bytes)]
struct SignatureResult {
    message: String,
    signature_hex: String,
}

// 定义多个签名结果的集合数据结构
#[derive(Archive, Serialize, Deserialize, Debug)]
#[archive(check_bytes)]
struct SignatureResults {
    results: Vec<SignatureResult>,
}

/// 解析RSA公、私钥的函数（只需执行一次）
fn parse_rsa_keys(private_key_der: &[u8], public_key_der: &[u8]) -> Result<(RsaPrivateKey, Vec<u8>), Box<dyn std::error::Error>> {
    // 打印私钥DER的十六进制形式
    println!("RSA.sk = {}", hex::encode(private_key_der));
    // 打印公钥DER的十六进制形式
    println!("RSA.PK = {}", hex::encode(public_key_der));
    // 从DER格式解析RSA私钥
    let private_key = RsaPrivateKey::from_pkcs8_der(private_key_der)?;
  
    Ok((private_key, public_key_der.to_vec()))
}

/// 生成RSA签名的函数
fn generate_rsa_signature(private_key: &RsaPrivateKey, message: &[u8]) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    // 计算消息的SHA-256哈希值
    let mut hasher = Sha256::new();
    hasher.update(message);
    let hashed_message = hasher.finalize();
    
    // 使用私钥和PKCS#1 v1.5填充方案生成签名
    let signature = private_key.sign(Pkcs1v15Sign::new::<Sha256>(), &hashed_message)?;
    
    Ok(signature)
}

fn main() {
    // 解析命令行参数
    let args = Args::parse();
    // 检查输入文件是否存在
    if !Path::new(&args.input_file).exists() {
        eprintln!("错误: 输入文件 {} 不存在", args.input_file);
        std::process::exit(1);
    }
    
    // 从文件读取多行消息
    let messages = match read_messages_from_file(&args.input_file) {
        Ok(messages) => messages,
        Err(err) => {
            eprintln!("读取文件失败: {}", err);
            std::process::exit(1);
        }
    };
    println!("从文件{}成功读取 {} 条message",args.input_file, messages.len());
    
    // 从内置资源加载2048位RSA私钥
    let private_key_der = include_bytes!("rsa2048-priv.der");
    let public_key_der = include_bytes!("rsa2048-pub.der");
    
    // 解析RSA密钥（只需执行一次）
    let (private_key, public_key) = match parse_rsa_keys(private_key_der, public_key_der) {
        Ok((priv_key, pub_key)) => (priv_key, pub_key),
        Err(err) => {
            eprintln!("RSA密钥解析失败: {}", err);
            std::process::exit(1);
        }
    };
    
    // 为每条消息生成RSA签名
    let mut results = Vec::new();
    
    // 记录开始时间
    let start_time = Instant::now();

    for (index, message) in messages.iter().enumerate() {
        
        // 生成RSA签名
        let result = generate_rsa_signature(&private_key, message.as_bytes())
            .map(|signature| {
                // 将签名转换为十六进制字符串
                let signature_hex = hex::encode(&signature);
                
                // 创建结果数据结构
                SignatureResult {
                    message: message.clone(),
                    signature_hex,
                }
            })
            .unwrap_or_else(|err| {
                eprintln!("消息 #{} 签名生成失败: {}", index + 1, err);
                std::process::exit(1);
            });
        
        if index == 1 {
            println!("The message = {}", result.message);
            println!("RSA.sig = {}", result.signature_hex);
        }
        
        // 添加到结果列表
        results.push(result);
    }
    
    // 记录结束时间
    let end_time = Instant::now();
    let duration = end_time.duration_since(start_time);
    println!("使用RSA算法对 {} 条消息进行签名, 耗时: {:?}", results.len(), duration);
    
    // 创建结果集合
    let signature_results = SignatureResults {
        results,
    };
    
    // 将结果使用rkyv序列化并写入文件
    match serialize_results(&signature_results, RSA_HASH_BATCH_FILE) {
        Ok(_) => {
            println!("所有结果已成功写入文件");
        },
        Err(err) => eprintln!("序列化或写入文件失败: {}", err),
    }
}

// 从文件读取多行消息
fn read_messages_from_file(file_path: &str) -> Result<Vec<String>, std::io::Error> {
    let file = File::open(file_path)?;
    let reader = BufReader::new(file);
    let mut messages = Vec::new();
    
    for line in reader.lines() {
        match line {
            Ok(message) => {
                // 跳过空行
                if !message.trim().is_empty() {
                    messages.push(message);
                }
            },
            Err(err) => return Err(err),
        }
    }
    
    Ok(messages)
}

// 使用rkyv将结果集合序列化并写入文件
fn serialize_results(results: &SignatureResults, file_path: &str) -> Result<(), std::io::Error> {
    // 使用rkyv序列化结果
    let bytes = rkyv::to_bytes::<_, 4096>(results)
        .map_err(|_| std::io::Error::new(std::io::ErrorKind::Other, "序列化失败"))?;
    
    // 打开或创建文件
    let mut file = File::create(file_path)?;
    
    // 写入序列化后的字节
    file.write_all(&bytes)?;
    
    Ok(())
}

// 从文件读取并使用rkyv反序列化结果集合
fn read_results(file_path: &str) -> Result<SignatureResults, std::io::Error> {
    // 检查文件是否存在
    if !Path::new(file_path).exists() {
        return Err(std::io::Error::new(std::io::ErrorKind::NotFound, "文件不存在"));
    }
    
    // 打开文件并读取所有字节
    let mut file = File::open(file_path)?;
    let mut bytes = Vec::new();
    file.read_to_end(&mut bytes)?;
    
    // 使用rkyv反序列化
    let deserialized = rkyv::from_bytes(&bytes)
        .map_err(|_| std::io::Error::new(std::io::ErrorKind::Other, "反序列化失败"))?;
    
    Ok(deserialized)
}