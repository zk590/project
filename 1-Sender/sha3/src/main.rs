...
fn main() {
    ...
    sha3.update(args.message.as_bytes());
    let hash = sha3.finalize();
    let hash_hex = hex::encode(hash);
    
    println!("SHA3-256哈希值: {}", hash_hex);
    ...
    // 将结果使用rkyv序列化并写入文件
    match serialize_and_write(&result, SHA3_HASH_FILE) {
        Ok(_) => {
            println!("结果已成功写入文件: {}", SHA3_HASH_FILE);
            
            // 测试反序列化功能
            match read_and_deserialize(SHA3_HASH_FILE) {
                Ok(deserialized) => {
                    println!("\n反序列化测试成功!");
