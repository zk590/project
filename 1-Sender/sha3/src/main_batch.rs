...
use common::constants::SHA3_HASH_BATCH_FILE;
...
fn main() {
    ...
    for (index, message) in messages.iter().enumerate() {
        ...
        if index==1 {
            println!("The SHA3.Preimage = {}",message);
            println!("The SHA3.hash = {}",hash_hex);
        }
        // 创建结果数据结构
    ...
    println!("使用SHA3算法对 {} 条消息进行哈希耗时: {:?}", messages.len(), duration);
    ...
    // 将结果使用rkyv序列化并写入文件
    match serialize_results(&hash_results, SHA3_HASH_BATCH_FILE) {
        Ok(_) => {
            println!("所有结果已成功写入文件");
        },
