...
pub struct LeafInfo {
    pub position: u64,
    pub leaf_hash: [u8; 32],
    pub proof_bytes: Vec<u8>, //节点路径
}

// 定义包含多个叶子节点信息的数据结构
#[derive(Archive, Serialize, Deserialize, Debug)]
#[archive(check_bytes)]
pub struct MultipleLeavesData {
    pub root_hash: [u8; 32],
    pub leaves_info: Vec<LeafInfo>,
}

// 使用rkyv将MultipleLeavesData序列化并写入文件
pub fn serialize_multiple_leaves_to_file(data: &MultipleLeavesData, file_path: &str) -> Result<(), std::io::Error> {
    // 使用rkyv序列化结果
...
pub fn read_and_deserialize_multiple_leaves(file_path: &str) -> Result<MultipleLeavesData, std::io::Error> {
    // 检查文件是否存在
...
///// 获取全局Merkle树的访问函数
pub fn get_global_merkle_tree() -> &'static Mutex<PoseidonTree::<(), { TREE_HEIGHT }>> {
    &GLOBAL_MERKLE_TREE
}

/// 从文件加载Merkle树状态
pub fn load_tree_state() -> Result<PoseidonTree::<(), { TREE_HEIGHT }>, std::io::Error> {
    use std::fs::File;
    use std::io::Read;
    use std::path::Path;
...
pub fn save_tree_state(tree: &PoseidonTree<(), { TREE_HEIGHT }>) -> Result<(), std::io::Error> {
    use std::fs::File;
    use std::io::Write;
...
pub fn initialize_merkle_tree(tree: &mut PoseidonTree<(), { TREE_HEIGHT }>, num_leaves: u64) -> Vec<u64> {
    // 创建一个随机数生成器
...
pub fn add_new_leaves(tree: &mut PoseidonTree<(), { TREE_HEIGHT }>, num_new_leaves: u64) -> Vec<u64> {
    // 创建一个随机数生成器
    let mut rng = StdRng::seed_from_u64(0xdea1);
...
/// 获取指定位置的叶子节点
pub fn get_leaf_at_position(tree: &PoseidonTree<(), { TREE_HEIGHT }>, position: u64) -> PoseidonItem<()> {
...
pub fn create_leaf_info(tree: &PoseidonTree<(), { TREE_HEIGHT }>, position: u64) -> LeafInfo {
