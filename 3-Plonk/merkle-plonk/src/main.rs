...
// 导入公共常量和电路定义
use common::constants::{TREE_HEIGHT, CAPACITY, CIRCUIT_PROVE_FILE, MERKLE_FILE, PLONK_PROOF_FILE, PLONK_PUBLICINPUTS_FILE, VERIFIER_FILE};

// 定义数据类型别名
type PoseidonItem = Item<()>;

/// 用于零知识证明的Merkle树路径电路,
// 该电路证明"我知道一个在特定位置的叶子节点，使得它在具有给定根节点的Merkle树中"
...
fn load_proof_data() -> Result<(u64, BlsScalar, BlsScalar, Opening<(), { TREE_HEIGHT }>), IoError> {
    ...
    let proof_data = unsafe {
        rkyv::archived_root::<MerkleProofData>(&bytes)
    };
    ...
    // 解析叶子节点哈希
    let leaf_hash = match BlsScalar::from_bytes(&proof_data.leaf_hash).into_option() {
        Some(hash) => hash,
        None => return Err(IoError::new(ErrorKind::Other, "解析叶子哈希失败"))
    };
    
    // 解析根哈希
    let root_hash = match BlsScalar::from_bytes(&proof_data.root_hash).into_option() {
        Some(hash) => hash,
    ...
    if opening.root().hash != root_hash {
        return Err(IoError::new(ErrorKind::Other, "证明中的根哈希与提供的根哈希不一致"));
    }
...
fn load_or_build_circuit() -> Result<(Prover, Verifier), Box<dyn std::error::Error>> {
    ...
    fs::write(CIRCUIT_PROVE_FILE, bincode::serialize(&prover_with_capacity)?)?;
...
fn generate_zk_proof() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 生成零知识证明
    let (proof, public_inputs) = prover.prove(&mut rand::thread_rng(), &circuit)?;
    println!("3. 生成零知识证明成功");
    ...
    let public_inputs_bytes_serialized = rkyv::to_bytes::<_, 1024>(&public_inputs_data)?;
    ...
    println!(" └── 公开输入数据大小: {} 字节", public_inputs_bytes_serialized.len());
...
pub fn verify_proof() -> Result<(), IoError> {
    ...
    let verifier: Verifier = Verifier::try_from_bytes(&verifier_bytes).map_err(|e| {
        IoError::new(ErrorKind::Other, format!("反序列化验证器失败: {:?}", e))
    })?;
    ...
    match load_proof_data() {
        Ok((position, leaf_hash, root_hash, opening)) => {
            ...
            if opening.root().hash == root_hash {
                ...
                println!("2. 根哈希一致性检查失败！证明中的根哈希与加载的根哈希不一致");
                return;
            }
            ...
            if is_valid {
                println!("4. 验证成功！叶子节点确实在具有给定根节点的Merkle树中");
            } else {
